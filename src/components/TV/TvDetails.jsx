...
export default function TvDetails() {
    ...
    async function GetTvDetails(mediaType, id) {
    ...
    async function GetVideoSource(id) {
        let { data } = await Axios.get(`https://api.themoviedb.org/3/tv/${id}/videos?api_key=a631b24dc97d4274715c5fa6b5a60321&language=en-US`);
        SetVideoSource(data.results.slice(0, 2))
    }
    let params = useParams()

    useEffect(() => {
        GetTvDetails('tv', params.id)
                ...
                {TvDetails ?
                    <>
                        <div className="col-md-4 mt-2">
                            <img src={'https://image.tmdb.org/t/p/w500/' + TvDetails.poster_path} className='w-100' />
                        </div>
                        <div className="col-md-8">
                            <h6 className='mt-5'> {TvDetails.title} </h6>
                            <p className='py-3'> {TvDetails.overview}</p>
                            <ul className='list-unstyled'>
                                <li>- Budgets : {TvDetails.budget}</li>
                                <li>- Vote : {TvDetails.vote_count}</li>
